/**
 * 日历插件
 * https://github.com/rookie125/mobile-calendar
 */
(function (factory) {
    ...
    var SLIDE_WIDTH = document.documentElement.clientWidth;
    ...
    var oCalenWrap = create('div', {
        ...
        calendarListContainer = create('div', {
        ...
        // 日历box
        calendarWeekList = create('div', {
            "class": 'calendar-week-list calendar-list'
        }),
        // 日历列表
        calenTitles,
        // 年，月标题
    ...
    function Calendar(options) {
        ...
        if (currentDate) date = new Date(currentDate);
        this.past = false;
        this.hours = false;
    ...
    // 设置为周模式
    Calendar.prototype.setWeekMode = function (e) {
        if (!this.isWeekMode) {
            (switchMonthWeekMode.bind(this))(-1);
        }
    };

    // 上周
    Calendar.prototype.preWeek = function (e) {
        this.setWeekMode();
        (switchMonth.bind(this))(calendarList, 1);
    };

    // 下周
    Calendar.prototype.nextWeek = function (e) {
        this.setWeekMode();
        (switchMonth.bind(this))(calendarList, -1);
    };

    // 上一天
    Calendar.prototype.preDay = function () {
        var span = $(calendarCurMonth).find('a.today').parent();
        span.prev().find('a').click();
        this.renderWeekList();
    };
    ...
    Calendar.prototype.init = function (options) {
        ...
        if (!(container || aCalendars.length)) return;

        oCalenWrap.appendChild(oCalenMask);
        oCalenWrap.appendChild(oCalen);
        (container || document.body).appendChild(oCalenWrap);
        if (container) {
            oCalenWrap.classList.add('hasContainer');
        ...
        this.createHeader(function () {
            ...
            calendarListContainer.appendChild(calendarList);
            calendarListContainer.appendChild(calendarWeekList);
            oCalen.appendChild(calendarListContainer);

            // 滑动切换上下月
            sildeSwitch(calendarList,
                switchMonth.bind(this),
                switchMonthWeekMode.bind(this), {
                    touchmoveCallback: function (dirs) {
                        ...
                        if (Math.abs(dir) > Math.abs(dirY)) {
                            ...
                            if (nextMonthFlag) {
                                calendarList.style.transform = 'translate(calc(-33.33% + ' + dir + 'px), 0)'
                            } else {
                                calendarList.style.transform = 'translate(calc(-33.33% + ' + dir + 'px), 0)'
                            }
                        } else {
                            // 纵向滑动
                            if (dirY < 0 && !self.isWeekMode) {
                                // 切换到周模式
                                calendarListContainer.style.height = (250 + dirY) + 'px';
                            }
                        }
                    },
                    touchendCallback: function (dirs) {
                        calendarList.style.transform = '';
                        calendarListContainer.style.height = '';
                    }
                });

            // 滑动切换上下周
            sildeSwitch(calendarWeekList,
                switchMonth.bind(this),
                switchMonthWeekMode.bind(this), {
                    touchmoveCallback: function (dirs) {
                        var dirY = dirs.dirY;
                        ...
                        if (Math.abs(dir) > Math.abs(dirY)) {
                            // 横向滑动
                            var nextWeekFlag = dir < 0;
                            if (nextWeekFlag) {
                                calendarWeekList.style.transform = 'translate(calc(-33.33% + ' + dir + 'px), 0)'
                            } else {
                                calendarWeekList.style.transform = 'translate(calc(-33.33% + ' + dir + 'px), 0)'
                            }
                        } else {
                            ...
                            if (dirY > 0 && self.isWeekMode) {
                                // 切换到月模式
                                calendarListContainer.style.height = (40 + dirY) + 'px';
                                calendarList.style.display = '-webkit-box';
                            }
                        }
                    },
                    touchendCallback: function (dirs) {
                        calendarWeekList.style.transform = '';
                        calendarWeekList.style.display = '';
                        calendarListContainer.style.height = '';
                    }
                });
        ...
        function initCa() {
            ...
            self.startDate = getDate(attribute(this, 'start-date') || options.currentDate || '');
                    ...
                    function (years) {
                        ...
                        sildeSwitch(selectYearBox,
                            function (obj, dir) {
                                ...
                                var val = 'translate(' + (selectYearBox.index * (100 / count)) + '%, 0)';
        ...
        for (var i = 0; i < aCalendars.length; i++) {
            ...
            aCalendars[i].removeEventListener('focus', initCa, false);
            aCalendars[i].addEventListener('focus', initCa, false);
        }
        if (container) {
            (initCa.bind(container))();
        }
    ...
    // 改变周模式transform
    Calendar.prototype.changeWeekModeTransform = function () {
        var weekDom = oCalenWrap.querySelectorAll('.calendar-week')[0];
        if (this.isWeekMode) {
            weekDom.classList.add('weekMode');
            calendarListContainer.classList.add('weekMode');

            var node = calendarList.querySelectorAll('a.today'),
                spanNode = node[0].parentElement;

            calendarList.style.transform = 'translate(-33.3333%, -' + spanNode.offsetTop + 'px)'

            $(calendarWeekList).show();
            $(calendarList).hide();
            this.renderWeekList();
        } else {
            weekDom.classList.remove('weekMode');
            calendarListContainer.classList.remove('weekMode');
            calendarList.style.transform = 'translate(-33.3333%, 0)'
    ...
    Calendar.prototype.createCalenList = function (data, setTitle) {
        ...
        var currentDate = this.currentDateObj.d;
        ...
        for (var i = 0; i < lastMonthDay - 1; i++) {

            var spanEle = create('span', {
                'data-date': [currentYear, seats(currentMonth - 1), seats(lastMonths[i])].join('-')
            }),
                dayEle = create('a', {
        ...
        for (var i = 0; i < dSun; i++) {
            ...
            var day = i + 1;
            var spanEle = create('span', {
                'data-date': [currentYear, seats(currentMonth), seats(day)].join('-')
            });
            var dayEle = create('a', {
            ...
            var time = getTime(currentYear, currentMonth, day);
            ...
            if (
                ...
                options.callback && options.callback(format(attribute(dayEle, 'data-calen'), attribute(dayEle, 'format') || false));
            }
        ...
        for (var i = 0; i < nextMonths; i++) {
            var day = i + 1;
            var spanEle = create('span', {
                'data-date': [currentYear, seats(currentMonth + 1), seats(day)].join('-')
            });
            var dayEle = create('a', {
     ...
     * 渲染周列表
     */
    Calendar.prototype.renderWeekList = function () {
        var diffArr = [-1, 0, 1];
        var _this = this;
        $(calendarWeekList).html('');
        diffArr.forEach(function (item) {
            var div = $('<div></div>');
            var arr = _this.getCurrentWeekRange(item);
            arr.forEach(function (dateItem) {
                var date = dateItem.date;
                var dom = $(calendarCurMonth).find('span[data-date="' + date + '"]');
                if (dom.length === 0) {
                    dom = $(calendarList).find('span[data-date="' + date + '"]');
                }
                var cloneDom = dom.clone();
                cloneDom.on('click', function () {
                    $(this).siblings().find('a').removeClass('today');
                    $(this).find('a').addClass('today');
                    dom.find('a').click();
                })
                div.append(cloneDom);
            })
            $(calendarWeekList).append(div);
        });
    }
    ...
    Calendar.prototype.createDate = function (data, callback) {
        ...
        if (data.type == 'year') {
            ...
            // 设置当前显示的页
            oDateList.style.WebkitTransform = 'translate(-' + (now * (100 / length)) + '%, 0)';
            oDateList.style.transform = 'translate(-' + (now * (100 / length)) + '%, 0)';
    ...
    Calendar.prototype.appendList = function (data, callback) {
        ...
        this.changeWeekModeTransform();

        // 渲染完列表后调用改变月份的回调
        var options = this.initOptions,
            curObj = this.currentDateObj;
        if (options.changeMonthCallback) {
            options.changeMonthCallback(this, curObj.y + '-' + seats(curObj.m) + '-' + seats(curObj.d));
        }
                        ...
                        if (self.currentNode) {
                            if (self.currentNode.value === undefined) {
                                if (options.container) {
                                    if (calendarList.querySelectorAll('a.today').length > 0) {
                                        calendarList.querySelectorAll('a.today')[0].classList.remove('today');
                                    }
                                    this.classList.add('today');
                                    options.callback && options.callback(date);
                                    return;
                                } else {
     ...
     */
    Calendar.prototype.transitions = function (obj, dir, callback) {
    ...
    // 获取当前日历显示的范围
    Calendar.prototype.getCurrentCalendarRange = function () {
        var currentDay = $(calendarList).find('a.today'),
            par = currentDay.parent(),
            list = par.parent(),
            start = list.children('span:eq(0)').attr('data-date'),
            end = list.children('span:eq(-1)').attr('data-date'),
            current = par.attr('data-date');

        return {
            startDate: start, endDate: end, currentDate: current
        }
    }

    // 获取当前日期范围的div
    Calendar.prototype.getCurrentCalendarList = function () {
        return $(calendarList).children('div:eq(1)').add($(calendarWeekList));
    }

    /**
     * 获取当前周的范围
     */
    Calendar.prototype.getCurrentWeekRange = function (weekDiff) {
        weekDiff = weekDiff || 0;
        var arr = [];
        var curDate = this.currentDateObj;
        var date = new Date(curDate.y, curDate.m - 1, curDate.d);
        var day = date.getDay();
        day = day === 0? 7:day;
        for (var i = 0; i < 7; i++) {
            var curDate = new Date(+date + (i + 1 + weekDiff * 7 - day) * 24 * 60 * 60 * 1000).Format('yyyy-MM-dd');
            arr.push({
                date: curDate,
                day: (i + 1) % 7,
                dayCn: ['一', '二', '三', '四', '五', '六', '日'][i],
            });
        }
        return arr;
    }
     ...
     * 获取绑定左右滑的函数
     */
    Calendar.prototype.getSildeSwitch = function () {
        return sildeSwitch;
    }

    /**
     * 滑动切换日期
     * @param  {[type]} ev [description] callBackVer(竖直滑动)
     * options 
     *      touchmoveCallback
     *      touchendCallback
     *      allowScrollY
     * @return {[type]}    [description]
     */
    function sildeSwitch(element, callBack, callBackVer, options) {
        options = options || {};
        var allowScrollY = options.allowScrollY;
        ...
        function start(evt) {
            ...
            var disX = oEv.pageX;
            var disY = oEv.pageY;
            var needW = parseInt(SLIDE_WIDTH, 10);
            var needH = parseInt(SLIDE_HEIGHT, 10);
            var dir;
            var dirY;
            ...
            function move(evt) {
                ...
                var allowYFlag = allowScrollY && Math.abs(dirY) > Math.abs(dir);

                if (sildeing) {
                    allowYFlag || (evt.preventDefault && evt.preventDefault());
                    return false;
                }

                options.touchmoveCallback && options.touchmoveCallback({ dir: dir, dirY: dirY });
                if (Math.abs(dir) >= needW) {
                    sildeing = true;
                } else if (Math.abs(dirY) >= needH) {
                    sildeing = true;
                }
                allowYFlag || (evt.preventDefault && evt.preventDefault())
                return false;
            ...
            function end(evt) {
                var oEv = evt.changedTouches ? evt.changedTouches[0] : (evt || event);
                dir = oEv.pageX - disX;
                dirY = oEv.pageY - disY;
                ...
                options.touchendCallback && options.touchendCallback({ dir: dir, dirY: dirY });

                if (Math.abs(dir) >= needW / 5 && Math.abs(dir) > Math.abs(dirY)) {
                    callBack && callBack(self, dir);
                }
                if (Math.abs(dirY) >= needH / 5 && Math.abs(dir) < Math.abs(dirY)) {
                    callBackVer && callBackVer(dirY);
                }
            }
    ...
    function format(dateStr, fmat) {
        ...
        var defaultFmat = fmat || 'yyyy-mm-dd';
    ...
    // 上个 月 或 周 
    function switchMonth(obj, dir) {
        var self = this;
        /**
         * 简单解释下，周模式下，如果没有翻月份，改变当前日期，并触发周模式transform即可
         */
        if (self.isWeekMode) {
            // 周模式
            var nextWeekFlag = dir < 0,
                curObj = self.currentDateObj,
                dateNow = curObj.d,
                curSpan = $(calendarList).find('a.today').parent(),
                index = curSpan.index(),
                par = curSpan.parent(),
                dateNext = new Date(+new Date(curObj.y, curObj.m - 1, curObj.d) + (nextWeekFlag ? 1 : -1) * 7 * 24 * 60 * 60 * 1000).getDate();

            if (nextWeekFlag) {
                // 下周
                if (dateNext > dateNow) {
                    par.find('span:eq(' + (index + 7) + ') a').click();
                    self.changeWeekModeTransform();
                    return;
                } else {
                    self.currentDateObj.d = dateNext;
                }
            } else {
                // 上周
                if (dateNext < dateNow) {
        ...
        self.startJSON.now.m = relativeMonth;
        self.startJSON.next.m = relativeMonth + 1;
        self.transitions(obj, dir, function () {
            self.renderWeekList();
        });
    }

    // 切换月和周模式
    function switchMonthWeekMode(dir) {
        var self = this;
        var initOptions = self.initOptions;
        if ((dir < 0 && self.isWeekMode) || (dir > 0 && !self.isWeekMode)) return;
        if (dir < 0) {
            // 变为周模式
            self.isWeekMode = true;
        } else {
            // 变为月模式
            self.isWeekMode = false;
        }
        self.changeWeekModeTransform();
        if (initOptions.switchMonthWeekModeCallback) {
            initOptions.switchMonthWeekModeCallback({ isWeekMode: self.isWeekMode })
        }
    }
